...
void* convoluteThread(void* threadArgs) {
    int pix,bit,span;
    struct ThreadArgs *my_thread = (struct ThreadArgs*)threadArgs;
    span=(*my_thread).src->bpp*(*my_thread).src->bpp;
    for (pix=0;pix<(*my_thread).src->width;pix++){
        for (bit=0;bit<(*my_thread).src->bpp;bit++){
...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    int thread_count = srcImage->height;
    ...
    for (row=0;row<srcImage->height;row++){
        ...
        for (pix = 0; pix < 3; pix++) {
            for (bit = 0; bit < 3; bit++) {
                (*threadArgs).alg[pix][bit] = algorithm[pix][bit];
            }
        }
        pthread_create(&thread_handles[row], NULL, convoluteThread, (void*) threadArgs);
    }
    free(thread_handles);
}
